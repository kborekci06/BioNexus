...
void app_error_fault_handler(uint32_t id, uint32_t pc, uint32_t info) {
    ...
    while(1) { bsp_board_led_invert(0); nrf_delay_ms(100); }
}
...
int main(void)
{
    ...
    const app_uart_comm_params_t comm_params = {
          ...
          APP_UART_FLOW_CONTROL_DISABLED, false, UART_BAUDRATE_BAUDRATE_Baud115200
    };
    APP_UART_FIFO_INIT(&comm_params, 256, 256, uart_error_handle, APP_IRQ_PRIORITY_LOWEST, err_code);
    APP_ERROR_CHECK(err_code);

    printf("\r\n--- BioNexus UWB Initiator Online ---\r\n");

    /* 3. Setup DW1000 IRQ pin */  
    nrf_gpio_cfg_input(DW1000_IRQ, NRF_GPIO_PIN_NOPULL);    
    
    /* 4. Reset DW1000 */
    reset_DW1000(); 

    /* 5. Set SPI clock to 2MHz for initial configuration */
    port_set_dw1000_slowrate();     
    
    /* 6. Init the DW1000 */
    if (dwt_initialise(DWT_LOADUCODE) == DWT_ERROR)
    {
        printf("UWB Radio Initialization Failed!\r\n");
        while (1) {};
    }
...
//   /*Initialization UART*/
//   // boUART_Init ();
//   /* Initialization UART (Bionexus Standard) */
//   uint32_t err_code;
//   const app_uart_comm_params_t comm_params = {
//         RX_PIN_NUMBER,
//         TX_PIN_NUMBER,
